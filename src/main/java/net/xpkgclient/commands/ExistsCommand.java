...
/**
 * This command checks if a file or directory exists.
 */
final class ExistsCommand extends TwoVarCommand {
    ...
    public static void execute(String[] args, ExecutionContext context) throws XPkgException {
        updateValues(CommandName.EXISTS, args, context);

        if (!(secondArg instanceof XPkgFile fileVar))
            throw new XPkgTypeMismatchException(CommandName.EXISTS, "second", VarType.FILE, secondArg.getVarType());
        context.setVar(assigneeName, new XPkgBool(fileVar.exists()));
    }
}
